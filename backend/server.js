    ...
    app.get("/tickets/open", (req, res) => {
        ...
        db.query(sql, (error, results) => {
            if (error){
                ...
                return res.status(500).json({error: "failed to get open ticktes"});
            }
            res.json(results);
        });
    });
...
app.post("/users", (req, res) => {
    ...
    // Password rule 1: Minium 8 characters
...
app.post("/tickets", async (req, res) =>{
    ...
    if (!title || !submitted_by){
        return res.status(400).json({ error: "title and submitted by are requred"});
    }
...
// POST / login - valadates credentials and returns user infor will role
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        await mongoDb.collection("activity_logs").insertOne({
            ...
            userId: user.id,
        ticket_id: null,
        details: `${user.first_name} ${user.last_nae} logged in as ${user.role}`,
        timestamp: new Date()
            });
    } catch(mongoError) {
        console.error("failed to log login activity:", mongoError);
        // Do not fail in the login if logging fails
    }
...
app.post("/ticket-notes", async(req, res)=> {
    ...
    if(!ticket_id || !note || !added_by) {
        return res.status(400).json({error: "ticket_id, note, added_by"});
    }try {
            const mongoDb = getMongo();
